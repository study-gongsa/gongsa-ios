...
//    let headers: HTTPHeaders = ["Authorization" : "Bearer \(KeychainSwift().get("access_key")!)"]
    ...
    // Header
    var token = "Bearer " + KeyChain.shared.read(key: "accessToken")!
//    if let acesstk = KeyChain.shared.read(key: "accessToken") {
//        self.token = "Bearer " + self.acesstk
//    }
    ...
    func getStudyGroupLists(completion : @escaping (NetworkResult<Any>) -> Void)
    {
        
//        let header : HTTPHeaders = ["Content-Type": "application/json"]
        let URL = "http://3.36.170.161:8080/api/study-group/my-ranking"
    ...
    private func judgeStatus(by statusCode: Int, _ data: Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(StudyGroupListsResponse.self, from: data) else { return .pathErr}
